...
def time_dependent_losses(R,hsat,xi,hOGS,wl,aT,aR,w0,f_atm,eta_int):
    ...
    for t in range(tmax,-tmax-1,-1):
        ...
        vals[count,1] = elev       # Elevation angle (rads)
    ...
    # Linear Loss
    #vals[:,2] = linear_loss(50,5,len(vals[:,0]))
    ...
    if loss_params['tReadLoss']:
        ...
        if loss_params['tWriteLoss']:
            loss_file = 'FS_loss_th_m_{:5.2f}_wl_{:.0f}nm_h_{}km_h1_{}km_aT_{}m_aR_{}m_w0_{}m.csv'.format(
                    np.degrees(theta_max)[0],loss_params['wvl'],
...
def linear_loss(max_val, min_val, array_length):
    if array_length < 2:
        raise ValueError("Array length must be at least 2.")
    
    step_up = (max_val*2 - min_val) / (array_length - 1)
    
    result = []
    for i in range(array_length):
        result.append(min_val + abs(i - (array_length - 1) / 2) * step_up)
    
    return 10**(-0.1*np.array(result))
