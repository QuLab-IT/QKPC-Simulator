...
def GaussianMode(r,w0):
    ...
    """
    return np.sqrt(2/np.pi) * np.exp(-(r/w0)**2) / w0
...
def diffract(aT,aR,z,k,w0):
    """
    Return the received to transmsitted power ratio. Calculated using the
    Fraunhoffer approximation to the Sommerfeld-Rayleigh integral
